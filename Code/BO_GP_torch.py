...
class GaussProcBayesOpt :
    ...
    def __call__(self, black_box_fn, X_0 : torch.Tensor, iterations : int, opt_type : str = 'MAX'):
        ...
        for _ in range(1,iterations) :
            ...
            Sigma[ [torch.tensor([i, n]) for i in range(n)] ] =  [ self.kernel_fn(x,x_i) for x in X ]
            Sigma[ [torch.tensor([n, i]) for i in range(n)] ] =  [ self.kernel_fn(x_i,x) for x in X ]
            X.reshape(n + 1)
            Y.reshape(n + 1)
            X[n], Y[n] = x_i, y_i
            n += 1
...
def gaussPHI(y_best : torch.Tensor, mu  : torch.Tensor, sigma  : torch.Tensor) :
    return -(torch.erf((y_best - mu) / (torch.sqrt(2) * sigma)) + 1) / 2
    
def gaussPhi(y_best : float, mu : torch.Tensor, sigma : torch.Tensor) :
    return torch.exp(-torch.square(y_best - mu) / (2 * torch.square(sigma))) / (torch.sqrt(2 * torch.pi) * sigma)

def mu(x : torch.Tensor, X : torch.Tensor, Y : torch.tensor, Sigma : torch.Tensor, kernel_fn) :
...
def acquisitionEI(mu : torch.Tensor, sigma : torch.Tensor, y_best : torch.Tensor) :
    return -( sigma * gaussPhi(y_best,mu,sigma) + (y_best - mu) * gaussPHI(y_best,mu,sigma) )

def acquisitionLCB(mu : torch.Tensor, sigma : torch.Tensor, kappa : torch.Tensor) :
    return mu - kappa * sigma

def jMu(x : torch.Tensor, X : torch.Tensor, Y : torch.tensor, Sigma : torch.Tensor, kernel_fn, J_kernel_fn) :
...
def jGaussPHI(y_best : torch.Tensor, mu  : torch.Tensor, sigma  : torch.Tensor, J_mu : torch.Tensor, J_sigma  : torch.Tensor) :
    return torch.exp(-torch.square((y_best - mu) / (torch.sqrt(2) * sigma))) * ( J_mu / sigma + (y_best - mu) * J_sigma / torch.square(sigma) ) / torch.sqrt(2 * torch.pi)

def jGaussPhi(y_best : torch.Tensor, mu  : torch.Tensor, sigma  : torch.Tensor, J_mu : torch.Tensor, J_sigma : torch.Tensor) :
    return ( torch.exp( -torch.square(y_best - mu) / ( 2 * torch.square(sigma) ) ) * sigma * ( (y_best - mu) * ( J_mu * sigma + (y_best - mu) * J_sigma ) / ( 2 * torch.pow(sigma,3) ) ) - torch.exp( -torch.square(y_best - mu) / ( 2 * torch.square(sigma) ) ) * J_sigma ) / ( torch.sqrt(2 * torch.pi) * torch.square(sigma) )

def jAcquisitionPI(y_best : torch.Tensor, J_mu : torch.Tensor, J_sigma : torch.Tensor) :
    return jGaussPHI(y_best,mu,sigma,J_mu,J_sigma)

def jAcquisitionEI(x : torch.Tensor, y_best : torch.Tensor) :
    return -( jSigma * gaussPhi(y_best,mu,sigma) + sigma * jGaussPhi(y_best,mu,sigma,jMu,jSigma) - jMu * gaussPHI(y_best,mu,sigma) + (y_best - mu) * jGaussPHI(y_best,mu,sigma,jMu,jSigma) )
