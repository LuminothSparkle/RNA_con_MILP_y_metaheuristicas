...
def main(args: argparse.Namespace):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for batch, start in enumerate(range(
            0, len(dataframe), args.batch_size
        )):
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--save_path', '-sp', type=Path, default=str(Path.cwd()))
    argparser.add_argument('--load_file', '-lf', type=Path, default=str(Path.cwd()))
