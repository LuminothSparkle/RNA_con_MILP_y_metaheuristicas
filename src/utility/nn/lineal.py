"""
Modulo que contiene la implemtación general de una arquitectura de red neuronal en Pytorch
"""
...
from torch.nn import (Sequential, Module, Linear, BatchNorm1d, Dropout, ReLU, ReLU6, PReLU,
                        ...
                        Identity, LogSigmoid, Softsign, ModuleDict)
from torch.optim import Optimizer
from torch.utils.data import DataLoader
from torch.masked import as_masked_tensor
from torch.nn.init import (xavier_normal_, xavier_uniform_, kaiming_uniform_, uniform_,
...
class LinealNN(Module):
    ...
    accepted_hyperparams = ['bias init', 'weight init', 'dropout', 'batch normalization',
                            'activation function', 'weight mask','connect dropout', 
                            'l1 weight regularization', 'l2 weight regularization',
                            'l1 activation regularization', 'l2 activation regularization']
    ...
    def __init__(self, C : Iterable[int], hyperparams : dict[str,Any] | None = None) :
        super().__init__()
        if hyperparams is None :
            hyperparams = {}
        self.capacity = list(C)
        self.layers = len(self.capacity) - 1
        self.hyperparams = { 'C' : list(C) }
        self.hyperparams['L'] =  len(self.hyperparams['C']) - 1
        for param in self.accepted_hyperparams :
            self.hyperparams[param] = {}
            if param in hyperparams and hyperparams[param] is not None :
                if (
                    isinstance(hyperparams[param], dict) and
                    any(isinstance(label,int) for label in hyperparams[param])
                ):
                    ...
                    self.hyperparams[param] = {k : hyperparams[param] for k in range(self.layers)}
        self._create_layers()

    def _create_layers(self) :
        """
        Function interna que genera los Module de cada capa en la red neuronal
        """
        self.bias_init = [
            self.hyperparams['bias init'][k]
            if k in self.hyperparams['bias init']
            else torch.ones
            for k in range(self.layers)
        ]
        self.sequential_layers = ModuleDict({
            str(k) : Sequential()
            for k in range(self.layers)
        })
        self.dropout_layers = ModuleDict({
            str(k) : Dropout(p = p, inplace = True)
            for k, p in self.hyperparams['dropout'].items()
        })
        self.batch_norm_layers = ModuleDict({
            str(k) : BatchNorm1d(num_features = self.capacity[k] + 1, **dict(kw))
            for k, kw in self.hyperparams['batch normalization'].items()
        })
        self.linear_layers = ModuleDict({
            str(k) : Linear(self.capacity[k] + 1,self.capacity[k + 1], bias = False)
            for k in range(self.layers)
        })
        self.activation_layers = ModuleDict({
            str(k) : self.functional_dict[name](**kw)
            for k, (name, kw) in
            self.hyperparams['activation function'].items()
        })
        for weight, init in (
            (self.linear_layers[str(k)].weight, init)
            for k, init in self.hyperparams['weight init'].items()
        ) :
            if isinstance(init,tuple) :
                ...
                weight = init
        for weight, mask in (
            (self.linear_layers[k].weight, mask)
            for k,mask in self.hyperparams['weight mask'].items()
        ) :
    ...
    def train_loop(self, dataloader : DataLoader, loss_fn : Callable[[Tensor,Tensor],Tensor],
                    optimizer : Optimizer, epochs : int,
        ...
        scheduler = extra_params['scheduler'] if 'scheduler' in extra_params else None
        early_tolerance = (
            extra_params['early tolerance']
            if 'early tolerance' in extra_params
            else None
        )
        ...
        if test_dataloader is not None :
            ...
            with torch.inference_mode() :
                self.eval()
                loss['start test'] = array( [loss_fn(self(X), y).item()
                                            for X,y in test_dataloader] )
                self.train()
            if verbose :
                print(f"start mean test loss : {numpy.mean(loss['start test']):>10g}")
        else :
            loss['test'] = None
        early_counter = 0
        self.train()
        for epoch in range(epochs) :
            for batch, (inputs, targets) in enumerate(dataloader) :
                ...
                batch_loss = loss_fn(self(inputs), targets)
                loss['train'][epoch,batch] = batch_loss.item()
                for k, weight in w_original.items() :
                    self.linear_layers[k].weight = weight
                ...
                optimizer.step()
            if verbose :
                self._print_verbose(epoch, 'mean train raw', loss['train'][epoch,:].mean())
                self._print_verbose(epoch, 'mean train norm', loss['normalized'][epoch,:].mean())
            if scheduler is not None :
                scheduler.step()
            if test_dataloader is not None :
                with torch.inference_mode() :
                    self.eval()
                    loss['test'][epoch,:] = array( [loss_fn(self(X), y).item() # type: ignore
                                                    for X,y in test_dataloader] )
                    loss['mean'][epoch] = loss['test'][epoch,:].mean() # type: ignore
                    self.train()
                if verbose :
                    self._print_verbose(epoch, 'mean test', loss['mean'][epoch])
            else :
    ...
    def inference(self, dataloader : DataLoader) :
        ...
        with torch.inference_mode() :
            self.eval()
            return array(self(X).item() for X,y in dataloader)

    def test_loop(self, dataloader : DataLoader, loss_fn : Callable[[Tensor,Tensor],Tensor]) :
        """
        Ciclo principal para calcular unicamente la perdida de los datos de prueba
        """
        with torch.inference_mode() :
            self.eval()
    ...
    def get_weights(self) :
        """
        Metodo que devuelve una lista de arreglos multidimesionales que contiene los pesos
        de las capas lineales en una red neuronal
        """
        with torch.inference_mode() :
            self.eval()
            weights = []
            for k,linear in self.linear_layers.items() :
                w, b = torch.tensor_split(
                                    ...
                                    (linear.weight.size(dim = 1) - 1,), # type: ignore
                                    dim = 1 )
    ...
    def set_weights(self, weights : list[ndarray]) :
        ...
        """
        self.batch_norm_layers = ModuleDict({})
        for k in self.sequential_layers :
            self.sequential_layers[k] = Sequential(*(
                module
