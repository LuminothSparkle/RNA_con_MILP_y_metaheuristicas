...
class LinealNN(Module):
    ...
    accepted_hyperparams = [
        'bias init', 'weight init', 'dropout', 'batch normalization',
        ...
        for param in self.accepted_hyperparams:
            ...
            if param in hyperparams and hyperparams[param] is not None:
                if (
                    ...
                    self.hyperparams[param] = {
                        k: data for k, data in enumerate(hyperparams[param])}
                else:
                    self.hyperparams[param] = {
                        k: hyperparams[param] for k in range(self.layers)}
        self._create_layers()
    ...
    def _create_layers(self):
        ...
        for k, batch in self.batch_norm_layers.items():
            self.sequential_layers[k].add_module(
                f'Batch normalization {k}', batch)
        for (k, seq), linear in zip(
            self.sequential_layers.items(),
            self.linear_layers.values()
        ):
    ...
    def forward(self, input_tensor: Tensor) -> Tensor:
        ...
        for k, layer in enumerate(self.sequential_layers.values()):
            layer_input = layer(torch.concat(
                (layer_input, bias[:, k].unsqueeze(dim=1)), dim=1))
    ...
    def train_loop(
        ...
        """
        Implementación del entrenamiento por propagacion hacia
        atras de una red neuronal
        """
        ...
        for epoch in range(epochs):
            ...
            if test_dataloader is not None:
                with torch.inference_mode():
                    self.eval()
                    loss['test'][epoch, :] = array([  # type: ignore
                        loss_fn(self(X), y).item()
                        for X, y in test_dataloader
                    ])
                    loss['mean'][epoch] = loss['test'][  # type: ignore
                        epoch, :
                    ].mean()
                    self.train()
                if verbose:
    ...
    def get_total_parameters(self):
        ...
        return numpy.sum([
            ...
            for linear in self.linear_layers.values()
        ])
