...
def simulated_annealing(
    black_box_fn: (...), x_0, y_0, temperature_fn: (...),
    init_temperature: float, neighbor_fn: (...),
    probability_fn: (...), iterations: int, opt_type: str
):
    ...
    for _ in range(iterations):
        ...
        if opt_type == (
            'MIN' and neighbor_y < y_best
            or opt_type == 'MAX' and neighbor_y > y
        ):
            x_best, y_best = neighbor_x, neighbor_y
        if probability_fn(y, neighbor_y, temperature, opt_type) > (
            torch.rand(1, 1)
        ):
